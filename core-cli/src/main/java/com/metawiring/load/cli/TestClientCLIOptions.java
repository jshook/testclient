...
public class TestClientCLIOptions {
    ...
    OptionSpec<String> phaseAndActivityDefs;
...

    {
        ...
        List<String> phasesAndActivities = asList("phase", "activity");
        phaseAndActivityDefs = parser.acceptsAll(phasesAndActivities).withRequiredArg().ofType(String.class)
                .required()
                .describedAs("Phase and Activities to run, formats: --phase=name or --activity=name:source:cycles:threads:asyncs:delay");
    ...
    public TestClientConfig parse(String[] args) {
        ...
        builder.withPhaseAndActivityDefs(options.valuesOf(phaseAndActivityDefs));
