 ...
 */
public class WriteRandomActivity extends BaseActivity {

    private static Logger logger = LoggerFactory.getLogger(WriteRandomActivity.class);
...
//            + " (" + SOURCE_FIELD + "," + YEARMONDAY_FIELD + "," + PARAM_FIELD + "," + TIMESTAMP_FIELD + "," + DATA_FIELD + ") values (?,?,?,?,?);";
    ...
    public void prepare(long startCycle, long endCycle, long maxAsync) {
        ...
        timerOps = context.getMetrics().timer(name(WriteRandomActivity.class.getSimpleName(), "ops-total"));
        timerWaits = context.getMetrics().timer(name(WriteRandomActivity.class.getSimpleName(), "ops-wait"));

        activityAsyncPendingCounter = context.getMetrics().counter(name(WriteRandomActivity.class.getSimpleName(), "async-pending"));

        triesHistogram = context.getMetrics().histogram(name(WriteRandomActivity.class.getSimpleName(), "tries-histogram"));
        ...
        if (addTelemetryStmt == null) {
            synchronized (WriteRandomActivity.class) {
                if (addTelemetryStmt == null) {
                    try {
                        if (addTelemetryStmt == null) {
...
// YCSB does 10 fields by default -- field0 .. field 9 blobs
// YCSB does 100 byte fields by default -- 1000 bytes total from 10 fields
