...
*/
package com.metawiring.load.cycler.inputs;
...
public class CycleSequenceSupplier implements ActivityInput {

    private AtomicLong min = new AtomicLong(0L);
    private AtomicLong max = new AtomicLong(Long.MAX_VALUE);

    public CycleSequenceSupplier setRange(long min, long max) {
        if (min>max) {
    ...
    public CycleSequenceSupplier setNextValue(long newValue) {
