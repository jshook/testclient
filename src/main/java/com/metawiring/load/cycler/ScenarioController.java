...
import com.metawiring.load.core.ActivityTypes;
...
public class ScenarioController {
    ...
    /**
     * Get the activity executor associated with the given alias. This should be used to find activities
    ...
    private ActivityExecutor getActivityExecutor(ActivityDef activityDef) {
        synchronized (activityExecutors) {
            ...
            if (executor == null) {
                ...
                ActivityType activityType = ActivityTypes.get(activityTypeName);
                executor = new ActivityExecutor(activityDef);

                ActivityInput input;
                if (activityType instanceof ActivityInput) {
                    input = (ActivityInput) activityType;
                } else {
                    input = new CycleSequenceSupplier().setRange(
                            activityDef.getStartCycle(),
                            activityDef.getEndCycle()
                    );
                }

                MotorDispenser amf;
                if (activityType instanceof MotorDispenser) {
                    amf = ((MotorDispenser) activityType);
                } else {
                    amf = new CoreMotorDispenser(
                            input,
                            activityType.getActionDispenser(activityDef)
                    );
                }

                executor.setActivityMotorDispenser(amf);
